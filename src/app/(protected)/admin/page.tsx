  ...
  return (
    ...
    >
      ...
      <motion.div className="h-full relative z-20 w-1/3 rounded-2xl bg-white p-4 dark:bg-black dark:border-white/[0.1] border border-neutral-200 flex flex-col items-center justify-center">
        <IconBrandVue className="rounded-full h-10 w-10 text-green-600" />
        <p className="sm:text-sm text-xs text-center font-semibold text-neutral-500 mt-4">
          我的组件库，基于 `Vuejs`
        </p>
        <Link
    ...
    >
      ...
      >
        ...
        <p className="text-xs text-neutral-500">
          ...
          Vue, Astro, Next.js that can make your life ....
        </p>
      </motion.div>
      <motion.div
        variants={variantsSecond}
...
const items = [
  {
    ...
    description: (
      <span className="text-sm">
        Release Viewer 是我在 Vercel 上部署的第 10 个应用。
      </span>
    ),
    header: <SkeletonOne />,
    className: "md:col-span-1",
    icon: <IconBrandVercel className="h-6 w-6" />,
  },
  {
    title: "Tailwind CSS",
    description: (
      <span className="text-sm">Release Viewer 通过 Tailwind CSS 所构建。</span>
    ),
    header: <SkeletonTwo />,
    className: "md:col-span-1",
    icon: <IconBrandTailwind className="h-6 w-6" />,
  },
  {
    title: "Next.js",
    description: (
  ...
  {
    title: "@PassionZale",
    description: <span className="text-sm">我还部署了一些更好玩的应用！</span>,
    header: <SkeletonFour />,
...

  {
    title: "PHP",
    description: (
      <span className="text-sm">对我来说 PHP 永远是世界上最好的语言！</span>
    ),
    header: <SkeletonFive />,
    className: "md:col-span-1",
    icon: <IconMoodXd className="h-6 w-6" />,
  },
];
    ...
    <PageContainer scrollable>
      <div className="space-y-4">
        <div className="flex items-center justify-between space-y-2">
          <h2 className="text-2xl font-bold tracking-tight">嗨, 欢迎~ 👋</h2>
        </div>

        <BentoGrid className="max-w-9/12 md:auto-rows-[20rem]">
          {items.map((item, i) => (
            <BentoGridItem
              key={i}
              title={item.title}
